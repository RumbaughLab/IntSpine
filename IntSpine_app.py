...
class SpineAnalyzerApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("SPINE: Volumetric Interactive Spine Analyzer")
    ...
    def init_ui(self):
        ...
        self.btn_analyze = QPushButton("Analyze All Targets")
        ...
        self.fig, (self.ax1, self.ax2) = plt.subplots(1, 2, figsize=(8.5, 5.5))
        self.fig.subplots_adjust(bottom=0.05, top=0.92, left=0.02, right=0.98, wspace=0.05)
        
        self.canvas = FigureCanvasQTAgg(self.fig)
        self.toolbar = NavigationToolbar2QT(self.canvas, self)
        self.toolbar.hide() # Hidden, we map custom buttons to it
    ...
    def create_slider(self, label, min_val, max_val, default, divisor=1):
        ...
        slider.setProperty('divisor', divisor) # Store for extraction later
    ...
    def connect_signals(self):
        ...
        self.btn_auto_seed.clicked.connect(self.auto_generate_seeds)
        ...
        self.btn_analyze.clicked.connect(self.on_analyze_all)
    ...
    def find_optimal_xyz(self, x, y, search_radius=5, start_z=None):
        ...
        y_min = max(0, y - search_radius); y_max = min(stack.shape[1], y + search_radius + 1)
        x_min = max(0, x - search_radius); x_max = min(stack.shape[2], x + search_radius + 1)
        ...
        z_min = max(0, best_z - 1); z_max = min(stack.shape[0], best_z + 2)
    ...
    def load_image_at_index(self, idx):
        ...
        self.ax2.set_xlim(-0.5, w - 0.5); self.ax2.set_ylim(h - 0.5, -0.5)
        ...
        self.z = 0
        self.mask = np.zeros_like(self.raw_stack, dtype=bool)
        self.is_drawing = False
    ...
    def refresh_display(self):
        ...
        if self.chk_segment.isChecked():
            self.mask_display.set_data(np.ma.masked_where(~self.mask[z], self.mask[z]))
            mip_m = np.max(self.mask, axis=0)
        ...
        if self.chk_markers.isChecked():
            if self.click_x is not None and self.rb_target.isChecked():
                ...
                self.click_marker.set_data([], [])
                self.click_marker_mip.set_data([], [])
            ...
            if self.target_z is not None and self.target_x is not None and self.rb_target.isChecked():
                ...
                self.target_marker_mip.set_data([], [])
    ...
    def paint(self, event):
        ...
        rr, cc = disk((y, x), self.val(self.lbl_brush), shape=self.painted_barrier_2d.shape)
        
        if self.rb_paint.isChecked():
            self.painted_barrier_2d[rr, cc] = True
            self.erased_barrier_2d[rr, cc] = False
        elif self.rb_erase.isChecked():
            self.erased_barrier_2d[rr, cc] = True
            self.painted_barrier_2d[rr, cc] = False
        ...
        if self.rb_paint.isChecked() or self.rb_erase.isChecked():
            ...
            if event.button == 3: # Right Click
                ...
                for t in self.saved_targets:
                    ...
                    if dist < min_dist:
                        min_dist, to_remove = dist, t
                if to_remove:
                    self.saved_targets.remove(to_remove)
                    self.update_list_ui()
                    self.refresh_display()
                    self.log(f"🗑️ Removed Target [{to_remove['idx']}] at X:{to_remove['click_x']}, Y:{to_remove['click_y']}")
                return
    ...
    def on_key_press(self, event):
        if event.key == 'z': self.on_save_target('spine')
        elif event.key == 'x': self.on_save_target('filopodia')
        elif event.key == 'c': self.on_save_target('suboptimal')
        elif event.key == 'u': self.on_undo_target()
        elif event.key in ['delete', 'backspace']: self.on_delete_selected_target()
        elif event.key == 'a': self.toolbar.home()
        elif event.key == 'f': self.toolbar.zoom()
        elif event.key == 'd': self.toolbar.pan()
        ...
        if self.click_x is not None and self.rb_target.isChecked():
            ...
            if not any(t['z'] == self.target_z and t['y'] == self.target_y and t['x'] == self.target_x for t in self.saved_targets):
                self.saved_targets.append({'idx': idx, 'label': label_text, 'z': self.target_z, 'y': self.target_y, 'x': self.target_x,
                                           'click_x': self.click_x, 'click_y': self.click_y, 'target_type': target_type, 'status': status})
                self.target_counter += 1
                self.update_list_ui()
                self.log(f"💾 Saved {target_type} {idx} at Z:{self.target_z+1}, Y:{self.target_y}, X:{self.target_x} ({status})")
                self.refresh_display()
    ...
    def on_delete_selected_target(self):
        ...
        if selected_items and self.saved_targets:
            for item in selected_items:
                ...
                if target_data: self.saved_targets.remove(target_data)
            self.update_list_ui()
            self.refresh_display()
            self.log(f"🗑️ Deleted {len(selected_items)} Target(s).")

    def on_rename_target(self):
        selected = self.list_targets.selectedItems()
        new_id_str = self.input_rename.text().strip()
        if len(selected) != 1:
            self.log("⚠️ Please select exactly ONE target from the list to update its ID.")
            return
        if not new_id_str.isdigit():
            self.log("⚠️ Please enter a valid number for the new ID.")
            return
        ...
        if target_data:
            ...
            self.refresh_display()
            self.log(f"✏️ Updated target to ID [{new_id}]")

    def on_apply_status(self):
        selected = self.list_targets.selectedItems()
        new_status = self.combo_status.currentText()
        if not selected:
            self.log("⚠️ Please select at least one target to apply status.")
            return
        ...
        self.update_list_ui()
        self.refresh_display()
        self.log(f"✅ Applied status '{new_status}' to {len(selected)} target(s).")
    ...
    def on_undo_target(self):
        if self.saved_targets:
            ...
            self.update_list_ui()
            self.refresh_display()
            self.log(f"↩️ Undid Target [{removed['idx']}]. Remaining: {len(self.saved_targets)}")
    ...
    def auto_generate_seeds(self):
        if self.raw_stack is None: return
        self.log_output.clear()
        self.log("🔍 Running background subtraction & geodesic filtering for auto-seeding...")
        
        self.saved_targets = []
        if self.input_custom_id.text().strip().isdigit():
            self.target_counter = int(self.input_custom_id.text().strip())
            self.input_custom_id.setText('')
        else: self.target_counter = 1
        ...
        if smoothed.max() > 0:
            ...
            eff_barrier_2d = np.max(self.get_effective_barrier(), axis=0)
            ...
            for i in range(1, num_features + 1):
                ...
                idx = self.target_counter; status = 'static'
                ...
                self.saved_targets.append({'idx': idx, 'label': label_text, 'z': opt_z, 'y': opt_y, 'x': opt_x,
                                           'click_x': x_loc, 'click_y': y_loc, 'target_type': 'spine', 'status': status})
                self.target_counter += 1
            
            self.update_list_ui()
            self.log(f"✅ Extracted {len(self.saved_targets)} seeds (Filtered out {filtered_count} objects > {max_geo_dist} µm away)!")
        self.refresh_display()
    ...
    def load_csv_seeds(self, seed_type):
        ...
        self.log_output.clear()
        if not matches:
            self.log(f"⚠️ No CSV file found matching '*{suffix}*.csv' in the folder.")
            return
            
        csv_file = matches[0]
        self.log(f"✅ Found CSV file: {os.path.basename(csv_file)}")
        try:
    ...
    def on_analyze_all(self):
        if not self.saved_targets or self.raw_stack is None:
            self.log_output.clear(); self.log("⚠️ Queue is empty. Save targets first.")
            return
            
        self.on_save_barrier()
        combined_mask = np.zeros_like(self.raw_stack, dtype=bool)
        results_list = []
        total_barrier = self.get_effective_barrier()
        current_smoothed_stack = np.where(total_barrier, 0, self.base_smoothed_stack)
        
        c_barrier = self.val(self.lbl_barrier)
        c_tol = self.val(self.lbl_tol)
        c_zsearch = self.val(self.lbl_zsearch)
        
        self.log("⚙️ Batch processing targets using 2.5D Dilated Envelope Extrusion...")
        ...
        for target in self.saved_targets:
            ...
            if t_type == 'filopodia':
                results_list.append({'Target_ID': idx, 'Classification': 'Filopodia', 'Status': target_status,
                                     'Z_Slice': z + 1, 'Original_Y': orig_y, 'Original_X': orig_x, 'Corrected_Y': y, 'Corrected_X': x, 
                                     'Vol_um3': 0.0, 'Geodesic_Distance_um': geo_dist})
                continue
                
            pad_xy, pad_z = 30, c_zsearch
            z_min_loc, z_max_loc = max(0, z - pad_z), min(self.raw_stack.shape[0], z + pad_z + 1)
            y_min_loc, y_max_loc = max(0, y - pad_xy), min(self.raw_stack.shape[1], y + pad_xy + 1)
            x_min_loc, x_max_loc = max(0, x - pad_xy), min(self.raw_stack.shape[2], x + pad_xy + 1)
            ...
            if seed_val == 0:
                self.log(f"❌ Target [{idx}] skipped: Seed is inside the pink dendritic barrier.")
                continue
            ...
            if labeled_2d[ly, lx] == 0:
                self.log(f"❌ Target [{idx}] skipped: 2D seed slice failed threshold or topology.")
                continue
                
            spine_mask_2d = binary_fill_holes(labeled_2d == labeled_2d[ly, lx])
            
            # --- 2. DILATED ENVELOPE EXTRUSION ---
            envelope_3d = np.broadcast_to(binary_dilation(spine_mask_2d, iterations=3), sub_stack.shape)
            ...
            if labeled_local[lz, ly, lx] == 0:
                self.log(f"❌ Target [{idx}] skipped: 3D region growing failed inside envelope.")
                continue
            ...
            if t_type == 'spine': combined_mask = np.logical_or(combined_mask, global_target_mask)
            
            vol = np.sum(global_target_mask) * self.voxel_volume
            ...
            results_list.append({'Target_ID': idx, 'Classification': class_label, 'Status': target_status,
                                 'Z_Slice': z + 1, 'Original_Y': orig_y, 'Original_X': orig_x, 'Corrected_Y': y, 'Corrected_X': x, 
                                 'Area_Opt_Z_um2': area_opt_z_um2, 'Geodesic_Distance_um': geo_dist, 'Vol_um3': vol,
                                 'Max_Intensity': int(self.raw_stack[global_target_mask].max()), 'Integrated_Density': int_density})
        ...
        final_results_df.to_csv(os.path.join(out_dir, base_name + '_spine_results.csv'), index=False)
        imwrite(os.path.join(out_dir, base_name + '_filtered.tif'), current_smoothed_stack)
        imwrite(os.path.join(out_dir, base_name + '_segmentation_mask.tif'), combined_mask.astype(np.uint8) * 255)
    ...
    def on_load_folder(self):
        if not os.path.isdir(self.input_folder):
            self.log_output.clear(); self.log("❌ Invalid input folder path.")
            return
        ...
        for f in all_files:
            ...
            if not os.path.exists(os.path.join(out_dir, bn + '_spine_results.csv')): remaining_files.append(f)
                
        if not remaining_files:
            self.log_output.clear(); self.log("🎉 All images in this folder have already been analyzed!")
            return
    ...
    def on_next_image(self):
        if self.current_idx < len(self.files) - 1:
            ...
            self.log_output.clear(); self.log("🎉 Completed all remaining files in the queue!")
